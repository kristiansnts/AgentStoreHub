...
interface ConnectWhatsAppScreenProps {
  onNext: (phone: string) => void;
  onBack?: () => void;
}

const ConnectWhatsAppScreen: React.FC<ConnectWhatsAppScreenProps> = ({ onNext, onBack }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-background-light dark:bg-background-dark">
      <Header title="Connect WhatsApp" showBack={true} onBack={onBack} />

      <main className="flex-1 overflow-y-auto px-6 pb-24 pt-8 no-scrollbar">
        <div className="flex flex-col items-center">
          <div className="mb-8 flex h-20 w-20 items-center justify-center rounded-2xl bg-green-50 text-green-600 shadow-sm dark:bg-green-900/20 dark:text-green-400">
            <Icon name="chat" className="text-[40px]" />
          </div>
          ...
          <div className="mb-10 text-center">
            ...
            <p className="text-text-sub dark:text-gray-400 text-sm leading-relaxed px-2">
              Connect your number to enable agent interactions directly through WhatsApp chat.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="w-full space-y-8">
            ...
            <p className="text-center text-xs text-text-sub dark:text-gray-500">
              By continuing, you agree to receive automated messages from AgentStore at the number provided.
            </p>
          </form>
        </div>
      </main>
      ...
      {isConfirming && (
        <div className="fixed inset-0 z-[100] mx-auto flex w-full max-w-md items-center justify-center px-4" role="dialog">
          ...
          <div className="relative w-full max-w-sm overflow-hidden rounded-xl bg-surface-light dark:bg-surface-dark p-6 text-center shadow-2xl ring-1 ring-black/5 dark:ring-white/10 transition-all animate-in zoom-in-95 duration-200">
            ...
            <h3 className="text-xl font-bold text-text-main dark:text-white">Confirm WhatsApp Number</h3>
            <div className="mt-2">
              <p className="text-xl font-semibold tracking-tight text-text-main dark:text-white py-1">{phone}</p>
              <p className="mt-2 text-sm leading-relaxed text-text-sub dark:text-gray-400">
                A one-time password (OTP) will be sent to this WhatsApp number to verify your account.
              </p>
            </div>
